...
try:
    client.collections["Nuntiatur Pius XI"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "Nuntiatur Pius XI",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "Nuntiatur Pius XI",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/nuntiaturberichte/nbr-pius-xi-static/{record['id']}"
...
print(make_index)
print("done with indexing Nuntiatur Pius XI")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index Nuntiatur Pius XI")
